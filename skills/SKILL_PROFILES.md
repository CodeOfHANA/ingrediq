# IngredIQ — Profile & Preset System Skill File

## User Identity
No auth system for hackathon. Use a self-generated user_id:
- Generate once on first app launch: str(uuid.uuid4())
- Persist in data/user_id.txt
- Load into st.session_state.user_id on every app start
...
profile = {
    "user_id": "uuid string",
    "name": "string",
    "presets": ["halal", "vegan"],           # list of active preset keys
    "medical_conditions": ["diabetes"],       # list of condition keys
    "allergies": ["nuts", "gluten"],          # list of allergen strings
    "medications": ["warfarin"],              # list of medication names
    "lab_values": {
        "blood_sugar_mmol": None,             # float or null
        "cholesterol_mmol": None,             # float or null
        "sodium_mmol": None,                  # float or null
        "creatinine_umol": None,              # float or null
        "potassium_mmol": None                # float or null
    },
    "preferences": ["low_sodium", "keto"],   # additional preferences
    "created_at": "ISO timestamp",
    "updated_at": "ISO timestamp"
}
```

## Preset Profiles — Full Rulesets
...
- When combining, merge ALL rules from all active presets
- Never drop any rule during merge
- Most restrictive rule always wins on conflict
- Example: Halal + Vegan → apply both full rulesets combined

## Profile Storage
- Primary: Supabase profiles table
- Fallback: data/profiles/local.json
- Always load into st.session_state.profile on app start
- Always save to both Supabase and local JSON on profile update

## UI Pattern for Preset Selection
- Show as clickable emoji + label badge cards
- Use st.columns(4) for badge grid
- Selected = colored border + checkmark overlay
- Unselected = gray border
- Allow multi-select freely
- Show active preset count in sidebar
